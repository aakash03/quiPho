	...
	function deny( uid ) {
		...
		<style>
			...
			.pageHeader {
			  ...
			  border-bottom: solid 2px #ff8366;
			}
			.pageHeader .title {
			  width: 40%;
			  float: left;
			...
			body {
			 background-image: url(http://onlineteachingtoolkit.com/images/orange-white-gradient.jpg);
			}
			a{
				color:white;
			}
...
<div id="wrap">
	...
	<div class="column column2">
		...
		<div class="content" id="shipping">
			<?php
				$res = mysqli_query($conn,"SELECT * FROM quiz WHERE userid IN ( SELECT bro_id FROM friends WHERE user_id = $userid AND status = 1 UNION SELECT user_id FROM friends WHERE bro_id = $userid AND status = 1 ) ORDER BY timestamp DESC");
				$cnt = $res->num_rows;
				if ($cnt==0) 
				{
					 echo "<label>No new activity from friends.</label>";
				}
				while( $row = mysqli_fetch_assoc($res) ) {
					$q = mysqli_query($conn, "SELECT * FROM user WHERE id = ".$row['userid']);
					$bro = mysqli_fetch_assoc($q);
					
					echo "<p>".$bro['name']." answered ".$row['question']." questions correctly  at ".$row['timestamp']."</p>";
				}
			?> 
		</div> 
	<div class="step" id="step2">
	...
	<div class="content" id="shipping">
			<?php
				$res = mysqli_query($conn,"SELECT * FROM quiz WHERE userid=$userid ORDER BY timestamp DESC");
				$cnt = $res->num_rows;
				if ($cnt==0) 
				{
					 echo "<label>You have not answered any quiz.</label>";
				}
				else
				{
					while( $row = mysqli_fetch_assoc($res) ) 
					{
						echo "<p> ".$row['question']."/5 at ".$row['timestamp']."</p>";
					}
				}
